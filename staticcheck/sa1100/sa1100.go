...
var SCAnalyzer = lint.InitializeAnalyzer(&lint.Analyzer{
	...
	Doc: &lint.RawDocumentation{
		Title:    "log.Fatalf should not be called in package not named `main`",
		Text:     `log.Fatalf calls os.Exit(1) which is unexpected in packages not named 'main'`,
		Since:    "Unreleased",
...
func run(pass *analysis.Pass) (interface{}, error) {
	...
	for _, file := range pass.Files {
		ast.Inspect(file, func(n ast.Node) bool {
			...
			// Check if the function is 'Fatalf' and from the 'log' package.
			if fun.Sel.Name == "Fatalf" {
				...
				if ok && pkgName.Imported().Path() == "log" {
					pass.Reportf(call.Pos(), "use of log.Fatalf in package not named 'main'")
				}
			}
			return true
		})
